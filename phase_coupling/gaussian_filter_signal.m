...
% % Debugging
% freqi = 1;
% varargin = {'output_type', 'analytic_signal',...
...
%       2) 'magnitude': abs(analytic_signal);       
%       3) 'power': real-valued [0,realmax) vector of instantaneous power of filtered signal at each sample point
%                power=abs(analytic_signal).^2;
%       4) 'complex_phase': complex-valued vector of unit length 
%                complex_phase=analytic_signal./abs(analytic_signal);
%       5) 'real_phase': real-valued vector (-pi,pi] vector of instantaneous phase of filtered signal at each sample point
%                real_phase=angle(analytic_signal);
%       6) 'real_signal': real-valued vector of instantaneous values of filtered signal
    ...
    switch lower(varargin{n})
        ...
        case 'raw_signal'
            ...
            number_of_fft_points=...
                2^ceil(log2(number_of_sample_points_in_signal));
            ffted_signal=...
                single(...
                fft(...
                raw_signal,...
...
frequency_step_size=sampling_rate/number_of_fft_points;
nyquist=sampling_rate/2;
exact_DFT_frequencies=single(frequency_step_size*...
...
if size(ffted_signal,1) ~= size(frequency_domain_gabor,1)
    frequency_domain_gabor = frequency_domain_gabor';
end
output_signal=...
    ifft(...
    ffted_signal.*...
...
switch output_type
    ...
    case 'magnitude'
        output_signal=abs(output_signal);
    case 'power'
        output_signal=abs(output_signal).^2;
    case 'complex_phase'
        output_signal=output_signal./abs(output_signal);
    case 'real_phase'
